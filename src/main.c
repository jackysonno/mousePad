...
// Thread function for delayed hooks
void *delayed_hook_thread(void *arg) {
    sleep(90); // 90 seconds delay before hooking
...
int32_t attr_public plugin_load(int32_t argc, const char *argv[]) {
    ...
    // Creating thread for delayed hooks instead of hooking directly
    pthread_t hook_tid;
    pthread_create(&hook_tid, NULL, delayed_hook_thread, NULL);
