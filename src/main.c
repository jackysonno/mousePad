...
static bool g_systemReady = false; // The master switch to prevent loading freezes
...
HOOK_DEFINE(scePadOpen, int32_t userId, int32_t type, int32_t index, void *param) {
    if (!g_systemReady) return HOOK_PASS(scePadOpen, userId, type, index, param);
...
HOOK_DEFINE(scePadClose, int32_t handle) {
    if (!g_systemReady) return HOOK_PASS(scePadClose, handle);
...
HOOK_DEFINE(scePadGetHandle, int32_t userId, uint32_t controller_type, uint32_t controller_index) {
    if (!g_systemReady) return HOOK_PASS(scePadGetHandle, userId, controller_type, controller_index);
...
HOOK_DEFINE(scePadReadState, int32_t handle, OrbisPadData *data) {
    if (!g_systemReady) return HOOK_PASS(scePadReadState, handle, data);
...
HOOK_DEFINE(scePadRead, int32_t handle, OrbisPadData *data, int32_t count) {
    if (!g_systemReady) return HOOK_PASS(scePadRead, handle, data, count);
...
HOOK_DEFINE(scePadGetControllerInformation, int32_t handle, OrbisPadInformation *info) {
    if (!g_systemReady) return HOOK_PASS(scePadGetControllerInformation, handle, info);
...
HOOK_DEFINE(scePadSetLightBar, int32_t handle, OrbisPadColor *inputColor) {
    if (!g_systemReady) return HOOK_PASS(scePadSetLightBar, handle, inputColor);
...
HOOK_DEFINE(scePadResetLightBar, int32_t handle) {
    if (!g_systemReady) return HOOK_PASS(scePadResetLightBar, handle);
...
HOOK_DEFINE(scePadSetVibration, int32_t handle, const OrbisPadVibeParam *param) {
    if (!g_systemReady) return HOOK_PASS(scePadSetVibration, handle, param);
...
HOOK_DEFINE(scePadResetOrientation, int32_t handle) {
    if (!g_systemReady) return HOOK_PASS(scePadResetOrientation, handle);
...
HOOK_DEFINE(scePadSetMotionSensorState, int32_t handle, bool enable) {
    if (!g_systemReady) return HOOK_PASS(scePadSetMotionSensorState, handle, enable);
...
HOOK_DEFINE(scePadSetTiltCorrectionState, int32_t handle, bool enable) {
    if (!g_systemReady) return HOOK_PASS(scePadSetTiltCorrectionState, handle, enable);
...
HOOK_DEFINE(scePadSetAngularVelocityDeadbandState, int32_t handle, bool enable) {
    if (!g_systemReady) return HOOK_PASS(scePadSetAngularVelocityDeadbandState, handle, enable);
...
HOOK_DEFINE(scePadDeviceClassParseData, int32_t handle, const OrbisPadData *data, OrbisPadDeviceClassData *classData) {
    if (!g_systemReady) return HOOK_PASS(scePadDeviceClassParseData, handle, data, classData);
...
HOOK_DEFINE(scePadDeviceClassGetExtendedInformation, int32_t handle, OrbisPadDeviceClassExtInfo *info) {
    if (!g_systemReady) return HOOK_PASS(scePadDeviceClassGetExtendedInformation, handle, info);
...
HOOK_DEFINE(sceUserServiceGetUserName, int32_t userId, char *username, size_t size) {
    if (!g_systemReady) return HOOK_PASS(sceUserServiceGetUserName, userId, username, size);
...
HOOK_DEFINE(sceUserServiceGetUserColor, int32_t userId, OrbisUserServiceUserColor *color) {
    if (!g_systemReady) return HOOK_PASS(sceUserServiceGetUserColor, userId, color);
...
HOOK_DEFINE(sceUserServiceGetLoginUserIdList, OrbisUserServiceLoginUserIdList *list) {
    if (!g_systemReady) return HOOK_PASS(sceUserServiceGetLoginUserIdList, list);
...
HOOK_DEFINE(sceUserServiceGetEvent, OrbisUserServiceEvent *event) {
    if (!g_systemReady) return HOOK_PASS(sceUserServiceGetEvent, event);
...
// Background thread that activates the virtual controller logic after 90 seconds
void *delayed_activation_thread(void *arg) {
    sleep(90); // Pure pass-through mode for 90 seconds during sensitive loading
    g_systemReady = true;
    final_printf("RemotePad: Safe mode deactivated. Virtual pads are now active!\n");
    Notify(TEX_ICON_SYSTEM, "RemotePad Active!"); // Show notification on PS4 screen
    return NULL;
}
...
int32_t attr_public plugin_load(int32_t argc, const char *argv[]) {
    ...
    // Apply ALL hooks immediately to capture scePadInit at second zero!
    HOOK32(scePadInit);
    HOOK32(scePadOpen);
    ...
    // Create the asynchronous background watchdog thread
